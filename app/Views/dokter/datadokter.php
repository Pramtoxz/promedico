    ...
    <div class="row">
        <div class="col-12">
            <div class="card card-teal">
                ...
                <div class="card-body">
                    <div class="buttons">
                        <a href="<?= site_url('dokter/formtambah') ?>" class="btn btn-danger">Tambah Data</a>
                    </div>
        ...
        <div class="modal-content" style="background-color: #f8f9fa; border-radius: 15px; box-shadow: 0 10px 20px rgba(0,0,0,0.19), 0 6px 6px rgba(0,0,0,0.23);">
            <div class="modal-header"
                ...
                <h5 class="modal-title" id="detailModalLabel">
                    <i class="fas fa-id-card mr-2"></i> Detail Dokter
                </h5>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formCreateUser">
                    <input type="hidden" id="user_id_dokter" name="id_dokter">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.sukses) {
                            ...
                            // Refresh DataTable
                            $('#tabelDokter').DataTable().ajax.reload();
                        } else {
                            Swal.fire({
                                title: 'Error!',
...
$(document).on('click', '.btn-edit', function() {
    var id_dokter = $(this).data('id_dokter');
    window.location.href = "<?php echo site_url('dokter/formedit/'); ?>" + id_dokter;
});
...
$(document).on('click', '.btn-detail', function() {
    ...
    $.ajax({
        type: "GET",
        url: "<?= site_url('dokter/detail/') ?>" + id_dokter,
        dataType: 'html',
        success: function(response) {
        ...
        error: function(xhr, status, error) {
            console.error(xhr.responseText);
            alert('Gagal memuat detail Dokter');
        }
    });
});
...
$('#btnSubmitCreateUser').on('click', function() {
    var id_dokter = $('#user_id_dokter').val();
    
    $.ajax({
        url: '<?= site_url('dokter/createUser/') ?>' + id_dokter,
        type: 'POST',
